...
export const useForm = (options = {}) => {
  ...
  const valid = computed(() =>
    Object.values(states).every((field) => !field.invalid)
  );

  const getInitialState = (field, initialValue, reset) => {
    return {
  ...
  const validateOn = async (option, defaultValue) => {
    ...
    isArray(options[option])
      ? (results = await validate(options[option]))
      : (options[option] ?? defaultValue) && (results = await validate());
    const fieldArr =
      Object.keys(fields).filter((field) => fields[field]?.options?.[option]) ||
      [];
  ...
  const validateFieldOn = (field, fieldOptions, option, defaultValue) => {
    (fieldOptions?.[option] ??
      isFieldValidate(field, options[option] ?? defaultValue)) &&
      validate(field);
  };
  ...
  const defineField = (field, fieldOptions) => {
    ...
    const props = mergeProps(
      resolve(fieldOptions, states[field])?.props,
      resolve(fieldOptions?.props, states[field]),
      {
        name: field,
        onBlur: () => {
        ...
        onInput: (event) => {
          states[field].value = event.hasOwnProperty("value")
            ? event.value
            : event.target.value;
        },
        onChange: (event) => {
    ...
    watch(
      () => states[field].value,
      (newValue, oldValue) => {
        if (states[field].reset) {
          setTimeout(() => delete states[field].reset, 1);
          return;
        }
    ...
    for (const [fieldName, fieldInst] of Object.entries(fields)) {
      if (flattenFields.includes(fieldName) || !field) {
        ...
        if (fieldResolver) {
          ...
          isArray(fieldResult.errors) &&
            (fieldResult.errors = { [fieldName]: fieldResult.errors });
  ...
  const reset = () => {
    Object.keys(states).forEach(
      (field) =>
        (fields[field].states = states[field] =
          getInitialState(field, fields[field]?.options?.initialValue, true))
    );
  };

  const update = (values) => {
    Object.keys(values).forEach(
      (field) => (states[field].value = values[field])
    );
  };
